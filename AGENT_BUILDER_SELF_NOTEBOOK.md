...
Use this file first after chat migration so the operator does not drift across many handoff fragments.
...
Before any serious repo/runtime claim or mutation:
...
Also check no active duplicate runtime:
...
## 2. Current proven repo state when this file was created

Head before this notebook commit:

    027407a

Remote delta before this notebook commit:

    0 / 0

Repo status before this notebook commit:

    clean

Latest important commits before this notebook:

    33ba04f fix: add AIMO proof pack anti-repeat guard
    027407a docs: add self files and BIOS migration index

## 3. What must be updated after important work

Primary rule from migration handoff:

    do not scatter new handoff files
    update this root notebook
    update operations/gpt_handoff/GPT_OPERATOR_JOURNAL.md when work is important
    if a formal next-chat handoff is needed, create it under operations/gpt_handoff, not tests/self_development

Existing handoff/catalog area:
    ...
    operations/gpt_handoff/P1_FULL_AUDIT_REPORT.md
    operations/gpt_handoff/P1_AUDIT_MACHINE_SUMMARY.json
    operations/gpt_handoff/GPT_ENTRYPOINT_COVERAGE_AUDIT.md
...
Owner wants Agent Builder as independent action machine:
    ...
    self-observation + self-change + self-verification + return-to-parent

Current branch of work is still:
...
The migration handoff says the correct cognitive route after current repair is:
...
Smallest cognitive atom -> validator -> negative tests -> AIMO wiring -> proof -> map refresh -> commit/push -> update this notebook.

Expected Answer Assimilator files if/when implemented:
...
This notebook replaces these duplicate ad-hoc handoff/index files:

    tests/self_development/AIMO_ORGAN_REPAIR_AND_CURRENT_STATE_20260717.md
    tests/self_development/AGENT_BUILDER_SELF_FILES_AND_BIOS_INDEX_20260717.md
    operations/reports/AIMO_ORGAN_REPAIR_AND_CURRENT_STATE_20260717.md
...
If a future handoff is needed, prefer updating this root notebook and operations/gpt_handoff/GPT_OPERATOR_JOURNAL.md instead of creating more scattered files.
