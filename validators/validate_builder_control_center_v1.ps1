...
$need=@('builder.status','repo.status','school.status','school.notification.plan','school.notification.send','agent.status','inventory.status','capability.status','memory.status','school.start','school.resume','agent.start','runtime.diagnose','memory.diagnose','inventory.diagnose','capability.diagnose','control_center.diagnose','builder.overview','remote_access.status','builder.preflight','builder.candidate.status','builder.acceptance.plan','builder.acceptance.verify','builder.run.status','builder.checkpoint.create');foreach($id in $need){if($ids -notcontains $id){throw "MISSING_ACTION:$id"}}
if($ids.Count -ne 26){throw 'ACTION_COUNT_BAD'}
...
$schoolStart=@($r.actions|Where-Object id -eq 'school.start')|Select-Object -First 1;if($null-eq$schoolStart-or$schoolStart.kind-ne'REMOTE_MUTATE'-or@($schoolStart.required_parameters).Count-ne1-or[string]$schoolStart.required_parameters[0]-ne'SchoolCount'){throw 'SCHOOL_START_COUNT_ONLY_REMOTE_CONTRACT_BAD'}
if(-not$ctlText.Contains("[ValidateSet('Test','Live')][string]`$SchoolMode='Live'")-or-not$ctlText.Contains('prepare_school_start_v1.ps1')){throw 'SCHOOL_START_AUTOPREP_CONTRACT_BAD'}
$prep='operations/control_center/prepare_school_start_v1.ps1';if(-not(Test-Path $prep)){throw 'SCHOOL_START_PREP_HELPER_MISSING'};$pt=$null;$pe=$null;[System.Management.Automation.Language.Parser]::ParseFile((Resolve-Path $prep),[ref]$pt,[ref]$pe)|Out-Null;if($pe.Count){throw 'SCHOOL_START_PREP_HELPER_PARSE_BAD'}
$ctlText=Get-Content $ctl -Raw;if(-not$ctlText.Contains('function Get-ManagedRunPhaseSummary')-or-not$ctlText.Contains('descendant_count=[int]$phase.descendant_count')-or-not$ctlText.Contains('active_phase=[string]$phase.active_phase')-or-not$ctlText.Contains('phase_evidence=[string[]]@($phase.phase_evidence)')){throw 'BUILDER_RUN_STATUS_PHASE_SUMMARY_MISSING'}
$ctlText=Get-Content $ctl -Raw;if($ctlText.Contains('descendant_command_lines')-or$ctlText.Contains('raw_command_line')){throw 'BUILDER_RUN_STATUS_RAW_COMMAND_OUTPUT_FORBIDDEN'}
...
$snAction=@($r.actions|Where-Object id -eq 'school.notification.plan')|Select-Object -First 1;if($null-eq$snAction-or$snAction.group-ne'DIAGNOSE'-or$snAction.kind-ne'PREPARE_ARTIFACT'-or-not[bool]$snAction.read_only-or-not[bool]$snAction.parallel_safe){throw 'SCHOOL_NOTIFICATION_PLAN_REGISTRY_CONTRACT_BAD'}
$sendAction=@($r.actions|Where-Object id -eq 'school.notification.send')|Select-Object -First 1;if($null-eq$sendAction-or$sendAction.group-ne'START'-or$sendAction.kind-ne'REMOTE_MUTATE'-or[bool]$sendAction.read_only-or[bool]$sendAction.parallel_safe){throw 'SCHOOL_NOTIFICATION_SEND_REGISTRY_CONTRACT_BAD'}
$resumeAction=@($r.actions|Where-Object id -eq 'school.resume')|Select-Object -First 1;if($null-eq$resumeAction-or$resumeAction.group-ne'START'-or$resumeAction.kind-ne'LIVE_MUTATE'-or[bool]$resumeAction.read_only-or[bool]$resumeAction.parallel_safe-or$resumeAction.canonical_entrypoint-ne'operations/school/run_agent_school.ps1'-or@($resumeAction.conflicts|Where-Object{$_ -in @('school.start','school.resume','agent.start')}).Count-ne3){throw 'SCHOOL_RESUME_REGISTRY_CONTRACT_BAD'}
$stopAction=@($r.actions|Where-Object id -eq 'school.stop')|Select-Object -First 1;if($null-eq$stopAction-or$stopAction.kind-ne'LIVE_MUTATE'-or[bool]$stopAction.read_only-or[bool]$stopAction.parallel_safe-or$stopAction.canonical_entrypoint-ne'operations/school/run_agent_school.ps1'){throw 'SCHOOL_STOP_REGISTRY_CONTRACT_BAD'}
$ctlText=Get-Content $ctl -Raw;$m=[regex]::Match($ctlText,'(?s)function Get-SchoolNotificationPlan\{(?<body>.*?)\r?\n\}');if(-not$m.Success-or-not$m.Groups['body'].Value.Contains('delivery_attempted=$false')-or$m.Groups['body'].Value.Contains('api.telegram.org')-or$m.Groups['body'].Value.Contains('Invoke-RestMethod')){throw 'SCHOOL_NOTIFICATION_PLAN_NO_SEND_BOUNDARY_BAD'}
if(-not$ctlText.Contains("kind -eq 'REMOTE_MUTATE'")-or-not$ctlText.Contains('TELEGRAM_BOT_TOKEN')-or-not$ctlText.Contains('TELEGRAM_CHAT_ID')-or-not$ctlText.Contains('TELEGRAM_API_OR_TRANSPORT_FAILURE')){throw 'SCHOOL_NOTIFICATION_SEND_CONTROLLER_CONTRACT_BAD'}
$ctlText=Get-Content $ctl -Raw;if($ctlText.Contains('ACTIONS=22|DIAGNOSE_ROUTES=10*')-or-not $ctlText.Contains('ACTIONS=\d+\|DIAGNOSE_ROUTES=\d+')){throw 'BUILDER_ACCEPTANCE_VERIFY_DYNAMIC_MARKER_BAD'}
...
$cc=@($dr.results|Where-Object id -eq 'control_center.diagnose')|Select-Object -First 1;if($cc.status-ne'DIAGNOSTIC_PASS'-or[int]$cc.action_count-ne26){throw 'CONTROL_DIAG_BAD'}
...
$conf=& $ctl -Mode Plan -Action @('school.start','school.resume','agent.start') -Json|ConvertFrom-Json;if($conf.status-ne'BLOCKED_CONFLICT'){throw 'START_CONFLICT_GATE_BAD'}
...
# school.resume isolated negative/confirmation-gate proof; does not launch School.
$resumeCtl=(Resolve-Path $ctl).Path;$resumeReg=(Resolve-Path $reg).Path
$resumeLab=Join-Path ([IO.Path]::GetTempPath()) ('efab_cc_resume_'+[guid]::NewGuid().ToString('N'));New-Item -ItemType Directory -Path $resumeLab -Force|Out-Null;$resumePushed=$false
try{
 git -C $resumeLab init -q;git -C $resumeLab config user.email 'validator@local';git -C $resumeLab config user.name 'validator'
 foreach($d in @('operations/control_center','.runtime/active_compact_semantic_memory_v1','.runtime/school_resume_v1')){New-Item -ItemType Directory -Path (Join-Path $resumeLab $d) -Force|Out-Null}
 Copy-Item $resumeReg (Join-Path $resumeLab 'operations/control_center/BUILDER_CONTROL_CENTER_REGISTRY_V1.json') -Force
 foreach($n in @('manifest.json','index.json')){[IO.File]::WriteAllText((Join-Path $resumeLab ('.runtime/active_compact_semantic_memory_v1/'+$n)),'{}',[Text.UTF8Encoding]::new($false))};[IO.File]::WriteAllText((Join-Path $resumeLab '.runtime/active_compact_semantic_memory_v1/cells.jsonl'),"{`"id`":`"lab`"}`n",[Text.UTF8Encoding]::new($false))
 git -C $resumeLab add .;git -C $resumeLab commit -q -m seed;Push-Location $resumeLab;$resumePushed=$true
 $noPending=& $resumeCtl -Mode Plan -Action @('school.resume') -Json|ConvertFrom-Json;if($noPending.status-ne'BLOCKED'-or@($noPending.readiness|Where-Object{$_.id-eq'school.resume' -and $_.reason-eq'No pending School request to resume'}).Count-ne1){throw 'SCHOOL_RESUME_NO_PENDING_NEGATIVE_BAD'}
 [IO.File]::WriteAllText((Join-Path $resumeLab '.runtime/school_resume_v1/pending_request.json'),'{bad-json',[Text.UTF8Encoding]::new($false));git add .;git commit -q -m malformed
 $badJson=& $resumeCtl -Mode Plan -Action @('school.resume') -Json|ConvertFrom-Json;if($badJson.status-ne'BLOCKED'-or@($badJson.readiness|Where-Object{$_.reason-eq'Pending School request JSON invalid'}).Count-ne1){throw 'SCHOOL_RESUME_BAD_JSON_NEGATIVE_BAD'}
 [IO.File]::WriteAllText((Join-Path $resumeLab '.runtime/school_resume_v1/pending_request.json'),'{"status":"DONE","count":37,"mode":"Test","topics":"x"}',[Text.UTF8Encoding]::new($false));git add .;git commit -q -m badstatus
 $badStatus=& $resumeCtl -Mode Plan -Action @('school.resume') -Json|ConvertFrom-Json;if($badStatus.status-ne'BLOCKED'){throw 'SCHOOL_RESUME_BAD_STATUS_NEGATIVE_BAD'}
 [IO.File]::WriteAllText((Join-Path $resumeLab '.runtime/school_resume_v1/pending_request.json'),'{"status":"PENDING","phase":"PAUSED_EXTERNAL","count":37,"mode":"Test","topics":"codex_school_task_template_strength"}',[Text.UTF8Encoding]::new($false));git add .;git commit -q -m validpending
 $readyResume=& $resumeCtl -Mode Plan -Action @('school.resume') -Json|ConvertFrom-Json;if($readyResume.status-ne'READY'){throw ('SCHOOL_RESUME_PLAN_READY_BAD:'+$readyResume.status)};$rr=@($readyResume.readiness|Where-Object id -eq 'school.resume')|Select-Object -First 1;if([int]$rr.parameters.Count-ne37-or$rr.parameters.Mode-ne'Test'-or$rr.parameters.Topics-ne'codex_school_task_template_strength'){throw 'SCHOOL_RESUME_PENDING_PARAMETERS_BAD'}
 $dummySchool=$null
 try{$dummySchool=Start-Process -FilePath 'cmd.exe' -ArgumentList @('/c','ping 127.0.0.1 -n 8 >nul & rem run_agent_school.ps1') -PassThru -WindowStyle Hidden;Start-Sleep -Milliseconds 500;$dupPlan=& $resumeCtl -Mode Plan -Action @('school.resume') -Json|ConvertFrom-Json;$dupReady=@($dupPlan.readiness|Where-Object id -eq 'school.resume')|Select-Object -First 1;if($dupReady.status-ne'ALREADY_RUNNING'){throw ('SCHOOL_RESUME_DUPLICATE_RUNTIME_GATE_BAD:'+$dupReady.status)}}finally{if($null-ne$dummySchool -and -not$dummySchool.HasExited){& taskkill.exe /PID $dummySchool.Id /T /F 2>$null|Out-Null;try{$dummySchool.WaitForExit(3000)}catch{}}}
 $proc0=@(Get-CimInstance Win32_Process|Where-Object{$_.CommandLine -and $_.CommandLine -match '(?i)run_agent_school\.ps1'}|Select-Object -ExpandProperty ProcessId);$gate=& $resumeCtl -Mode Run -Action @('school.resume') -Json|ConvertFrom-Json;if($gate.status-ne'BLOCKED_CONFIRMATION_REQUIRED'){throw ('SCHOOL_RESUME_CONFIRM_GATE_BAD:'+$gate.status)};$proc1=@(Get-CimInstance Win32_Process|Where-Object{$_.CommandLine -and $_.CommandLine -match '(?i)run_agent_school\.ps1'}|Select-Object -ExpandProperty ProcessId);if(@($proc1|Where-Object{$proc0 -notcontains $_}).Count){throw 'SCHOOL_RESUME_NEGATIVE_STARTED_RUNTIME'}
}finally{if($resumePushed){Pop-Location};for($cleanupAttempt=0;$cleanupAttempt-lt12 -and (Test-Path $resumeLab);$cleanupAttempt++){try{Remove-Item $resumeLab -Recurse -Force -ErrorAction Stop}catch{Start-Sleep -Milliseconds 250}};if(Test-Path $resumeLab){throw 'SCHOOL_RESUME_LAB_CLEANUP_FAILED'}}
...
Write-Output ('PASS_BUILDER_CONTROL_CENTER_V4|ACTIONS=26|DIAGNOSE_ROUTES=11|OVERVIEW=PASS|REMOTE_SCOPE=PASS|MULTI_DIAG=PASS|LIVE_MUTATION=0|TRACKED_MUTATION=0|RUNTIME_STARTED=0')
