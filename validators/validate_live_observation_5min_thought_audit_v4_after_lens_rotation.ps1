...
$report='operations/autonomous_inner_motor/reports/LIVE_OBSERVATION_5MIN_THOUGHT_AUDIT_V4_AFTER_LENS_ROTATION.json'
if(-not(Test-Path $report)){ Add-Err "missing_report:$report" }
$r=$null
if(Test-Path $report){ $r=Get-Content $report -Raw|ConvertFrom-Json }
if($r){
  if($r.status -ne 'PASS_LIVE_OBSERVATION_5MIN_THOUGHT_AUDIT_V4_AFTER_LENS_ROTATION'){ Add-Err "status:$($r.status)" }
  ...
  if(@($r.observations.active_goal_lenses | Where-Object { $_.name -eq 'counterexample' -and $_.count -eq 4 }).Count -ne 1){ Add-Err 'counterexample_active_4_missing' }
  if(@($r.observations.active_goal_lenses | Where-Object { $_.name -eq 'boundary_condition' -and $_.count -eq 4 }).Count -ne 1){ Add-Err 'boundary_active_4_missing' }
  if(@($r.observations.selected_lenses | Where-Object { $_.name -eq 'counterexample' -and $_.count -eq 4 }).Count -ne 1){ Add-Err 'counterexample_selected_4_missing' }
  if(@($r.observations.selected_lenses | Where-Object { $_.name -eq 'boundary_condition' -and $_.count -eq 4 }).Count -ne 1){ Add-Err 'boundary_selected_4_missing' }
  ...
  if($r.observations.safety.any_direct_active_memory_write -ne $false){ Add-Err 'direct_active_memory_write_true' }
  if([double]$r.bloat.new_run_total_mb -lt 7){ Add-Err "new_run_total_too_low:$($r.bloat.new_run_total_mb)" }
  if($r.recommended_next.technical -ne 'LENS_AWARE_MEMORY_RETRIEVAL_V1'){ Add-Err "next_wrong:$($r.recommended_next.technical)" }
}
$procs=@(Get-CimInstance Win32_Process | Where-Object { $_.ProcessId -ne $PID -and $_.CommandLine -and $_.CommandLine -notmatch 'Get-CimInstance Win32_Process|validate_live_observation_5min_thought_audit_v4_after_lens_rotation.ps1' -and $_.CommandLine -match '\s-File\s+.*(start_agent_life_v1.ps1|run_autonomous_inner_motor.ps1|run_continuous_agent_runtime_v1_lab.ps1|school|invoke_body_self_inspection_circuit_v1.ps1)|codex exec|node_modules.*@openai/codex|node.*codex.js|continuous_agent_runtime_v1|validate_|live_observation' })
if($procs.Count -ne 0){ Add-Err "process_count_not_zero:$($procs.Count)" }
$status=if($errors.Count -eq 0){'PASS_LIVE_OBSERVATION_5MIN_THOUGHT_AUDIT_V4_AFTER_LENS_ROTATION'}else{'FAIL_LIVE_OBSERVATION_5MIN_THOUGHT_AUDIT_V4_AFTER_LENS_ROTATION'}
$proof=[ordered]@{schema='live_observation_5min_thought_audit_v4_validation';status=$status;checked_at=(Get-Date).ToUniversalTime().ToString('o');report=$report;cycle_count=if($r){$r.observations.cycle_count}else{$null};lens_rotation_live_visible=$true;active_goal_lenses=if($r){$r.observations.active_goal_lenses}else{@()};selected_lenses=if($r){$r.observations.selected_lenses}else{@()};unique_ref_sets=if($r){$r.observations.unique_ref_sets}else{$null};new_run_total_mb=if($r){$r.bloat.new_run_total_mb}else{$null};process_count=$procs.Count;errors=@($errors);boundary=[ordered]@{audit_only=$true;runtime_launched_by_validator=$false;active_memory_mutated_by_validator=$false;repo_mutation_by_validator=$false}}
WJson 'tests/self_development/LIVE_OBSERVATION_5MIN_THOUGHT_AUDIT_V4_AFTER_LENS_ROTATION_PROOF.json' $proof
