...
if($ids.Count -ne 24){throw 'ACTION_COUNT_BAD'}
...
$list=& $ctl -Mode List -Json|ConvertFrom-Json;if(@($list).Count -ne 24){throw 'LIST_ACTION_COUNT_BAD'}
...
$cc=@($dr.results|Where-Object id -eq 'control_center.diagnose')|Select-Object -First 1;if($cc.status-ne'DIAGNOSTIC_PASS'-or[int]$cc.action_count-ne24){throw 'CONTROL_DIAG_BAD'}
...
Write-Output ('PASS_BUILDER_CONTROL_CENTER_V4|ACTIONS=24|DIAGNOSE_ROUTES=11|OVERVIEW=PASS|REMOTE_SCOPE=PASS|MULTI_DIAG=PASS|LIVE_MUTATION=0|TRACKED_MUTATION=0|RUNTIME_STARTED=0')
