...
Write-Output ('PASS_CAPABILITY_INVOCATION_MAP_V1_DRAFT|TASKS='+$tasks.Count+'|CAPABILITIES='+@($m.capabilities).Count+'|NO_CAPABILITY_ID='+$withoutId.Count+'|RETIRED='+[int]$m.coverage.retired_tasks_accounted+'|ACCOUNTED='+[int]$m.coverage.accounted_total+'|LIVE_PROVEN='+@($m.capabilities|Where-Object{$_.live_or_lab_status-eq'PROVEN_LIVE'}).Count+'|INVOCABLE='+@($m.capabilities|Where-Object{@($_.invocation_modes).Count-gt0}).Count+'|WORKTREE_MUTATION=0|MEMORY_MUTATION=0')
