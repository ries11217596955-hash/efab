...
function New-InnateReflexBootload([string]$RunRoot,[string]$OutputPath){
  ...
  if($bootload.body_audit_reflex.status -ne 'DEFAULT_WAKE_OBSERVE'){ throw "BODY_AUDIT_REFLEX_STATUS_MISMATCH:$($bootload.body_audit_reflex.status)" }
  ...
  if($bootload.body_audit_reflex.callable -ne $true){ throw 'BODY_AUDIT_REFLEX_MUST_BE_CALLABLE_FOR_WAKE_OBSERVE' }
...
function Invoke-DefaultWakeReflexes([string]$RunRoot,$Bootload,[string]$OutputPath){
  ...
  $signalPath=Join-Path $bodyRuntimeRoot 'body_self_inspection_signal.json'
  $parentPacketPath=Join-Path $bodyRuntimeRoot 'body_self_inspection_parent_packet.json'
  $circuitProofPath=Join-Path $bodyRuntimeRoot 'BODY_SELF_INSPECTION_CIRCUIT_PROOF.json'
  foreach($required in @($signalPath,$parentPacketPath,$circuitProofPath)){ if(-not(Test-Path -LiteralPath $required -PathType Leaf)){ throw "BODY_AUDIT_WAKE_OUTPUT_MISSING:$required" } }
  $signal=Get-Content -Raw -LiteralPath $signalPath | ConvertFrom-Json
  $parentPacket=Get-Content -Raw -LiteralPath $parentPacketPath | ConvertFrom-Json
  $circuitProof=Get-Content -Raw -LiteralPath $circuitProofPath | ConvertFrom-Json
...
function Get-MentalFrontierRouter($ExpansionGate){
  ...
  foreach($candidate in $candidates){
    ...
    } elseif($candidate -eq 'knowledge_source_gap'){
      $score=3
      $reason='new_knowledge_frontier_requires_source_gap_detection'
    } elseif($candidate -eq 'validator_proof_gap'){
      $score=2
      $reason='proof_layer_can_strengthen_reuse_claims'
    }
    $scores += [ordered]@{ frontier=[string]$candidate; score=$score; reason=$reason }
  }
  $ordered=@($scores | Sort-Object score -Descending)
  $selected=$ordered | Select-Object -First 1
  return [ordered]@{
    schema='mental_frontier_router_v1'
    status=if($selected){'PASS_MENTAL_FRONTIER_ROUTER_V1'}else{'BLOCKED_MENTAL_FRONTIER_ROUTER_NO_CANDIDATES'}
    routed_at=(Get-Date).ToUniversalTime().ToString('o')
    ...
    route_instruction=if($selected){'Next mental-growth cycle should generate knowledge/atom candidates for selected_frontier and avoid returning to saturated topic unless new evidence appears.'}else{'No frontier route available.'}
    boundary=[ordered]@{ action_execution_allowed=$false; direct_active_memory_write=$false; router_only=$true; no_codex_launch=$true; no_web_research=$true; no_repo_repair_execution=$true }
  }
}
...
function New-DynamicMemoryRetrievalBudget($RunId,$InternalGoal,$DefaultWakeReflexes,$NewThoughtSeedToActiveGoal,$RecentThoughtRepetitionPattern){
  $base=7
  $target=$base
  $reason='fallback fixed top-window'
  $seedConsumed=($NewThoughtSeedToActiveGoal -and $NewThoughtSeedToActiveGoal.seed_consumed -eq $true)
  $repeatDetected=($RecentThoughtRepetitionPattern -and $RecentThoughtRepetitionPattern.repeat_detected -eq $true)
  $activeMemoryReady=$false
  if($DefaultWakeReflexes -and $DefaultWakeReflexes.active_memory_read_reflex){
    $r=$DefaultWakeReflexes.active_memory_read_reflex
    $activeMemoryReady=($r.root_exists -eq $true -and $r.manifest_exists -eq $true -and $r.index_exists -eq $true -and $r.cells_exists -eq $true)
  }
  if($seedConsumed){
    $target=5
    $reason='refocus seed consumed; use focused memory budget instead of fixed 7'
  } elseif($repeatDetected){
    $target=4
    $reason='recent repeat detected; use narrow memory budget to avoid same retrieval loop'
  }
  if(-not $activeMemoryReady){
    $target=0
    $reason='active memory not ready; retrieval blocked'
  }
  return [ordered]@{ schema='dynamic_memory_retrieval_budget_v1'; status='PASS_DYNAMIC_MEMORY_RETRIEVAL_BUDGET_V1'; run_id=$RunId; base_limit=$base; target_count=[int]$target; min_count=0; max_count=9; reason=$reason; inputs=[ordered]@{ seed_consumed=[bool]$seedConsumed; repeat_detected=[bool]$repeatDetected; active_goal_source=if($InternalGoal){$InternalGoal.source}else{$null}; active_memory_ready=[bool]$activeMemoryReady; active_memory_read_reflex_status=if($DefaultWakeReflexes -and $DefaultWakeReflexes.active_memory_read_reflex){$DefaultWakeReflexes.active_memory_read_reflex.status}else{$null}; runtime_pressure_reflex_status=if($DefaultWakeReflexes -and $DefaultWakeReflexes.runtime_pressure_reflex){$DefaultWakeReflexes.runtime_pressure_reflex.status}else{$null} }; boundary=[ordered]@{ budget_only=$true; thinking_only=$true; no_active_memory_write=$true; no_repo_mutation=$true; no_action_execution=$true; no_new_store_created=$true } }
}
function New-SelectiveCompactMemoryRetrieval($RunId,$InternalGoal,$MindLogic,$MentalFrontierRouter,$DynamicMemoryRetrievalBudget,[string]$ActiveMemoryRoot='.runtime/active_compact_semantic_memory_v1'){
  ...
  $budgetLimit=7
  if($DynamicMemoryRetrievalBudget -and $DynamicMemoryRetrievalBudget.target_count -ge 0){ $budgetLimit=[int]$DynamicMemoryRetrievalBudget.target_count }
  $top=if($budgetLimit -gt 0){ @($selected|Sort-Object relevance_score -Descending|Select-Object -First $budgetLimit) } else { @() }
...
function New-ShortTermStateToNextTaskRouter($RunId,$ShortTermMindState,$DecisionSpine,$SelectiveCompactMemoryRetrieval,$PreviousShortTermMindState,$RecentThoughtRepetitionPattern){
  ...
  if($ShortTermMindState){
    ...
    $signals += [ordered]@{ signal='active_thread_state'; value=$ShortTermMindState.active_thread.state; weight=0.9 }
    $signals += [ordered]@{ signal='completed_candidate_route_status'; value=$ShortTermMindState.completed_candidate.route_status; weight=0.8 }
    $signals += [ordered]@{ signal='selected_memory_ref_count'; value=$ShortTermMindState.active_thread.selected_memory_ref_count; weight=0.7 }
  }
  if($PreviousShortTermMindState -and $PreviousShortTermMindState.found){
    $signals += [ordered]@{ signal='previous_short_term_state_found'; value=$true; weight=0.8 }
    $signals += [ordered]@{ signal='previous_candidate_route_status'; value=$PreviousShortTermMindState.candidate_route_status; weight=0.7 }
  }
  if($DecisionSpine){
  ...
  $selectedTask='FRONTIER_TO_BUILD_TASK_ROUTER_V1'
  $selectedTaskHuman='Turn selected thought or frontier into a concrete build task with files, validator, and proof.'
  ...
  if($RecentThoughtRepetitionPattern -and $RecentThoughtRepetitionPattern.repeat_detected){
    ...
    $priorityScore=0.9
  } elseif($DecisionSpine -and $DecisionSpine.next_action_type -eq 'BLOCKED_NEEDS_MEMORY_RETRIEVAL'){
    $selectedTask='SELECTIVE_COMPACT_MEMORY_RETRIEVAL_REPAIR_V1'
    $selectedTaskHuman='Repair compact memory retrieval before next action selection.'
    $reason='decision spine still blocked on memory retrieval'
    $priorityScore=0.88
  } elseif($PreviousShortTermMindState -and -not $PreviousShortTermMindState.found){
    $selectedTask='SHORT_TERM_STATE_CONTINUITY_STRENGTHENING_V1'
    $selectedTaskHuman='Strengthen continuity between cycles.'
    $reason='no prior short-term state found, so continuity is weak'
    $priorityScore=0.68
  }
  return [ordered]@{
    schema='short_term_state_to_next_task_router_v1'
    status='PASS_SHORT_TERM_STATE_TO_NEXT_TASK_ROUTER_V1'
    run_id=$RunId
    selected_next_task=$selectedTask
    selected_next_task_human=$selectedTaskHuman
    task_type=$taskType
    ...
    candidate_task_contract=[ordered]@{
      ...
      required_inputs=@('short_term_mind_state','decision_spine','selective_compact_memory_retrieval','existing_multi_source_warehouse_signal')
      expected_outputs=@('task_contract','validator_target','proof_target')
      suggested_validator='validators/validate_frontier_to_build_task_router_v1.ps1'
      suggested_proof='tests/self_development/FRONTIER_TO_BUILD_TASK_ROUTER_V1_PROOF.json'
...
function New-BuildTaskBoundedExecutor($RunId,$BuildTaskContractExecutionGate,$FrontierToBuildTaskRouter){
  $contract=$null
  if($FrontierToBuildTaskRouter -and $FrontierToBuildTaskRouter.contract){ $contract=$FrontierToBuildTaskRouter.contract }
  ...
  if($gateAllowsDryRun){
    ...
    if($contract){
      foreach($w in @($contract.files_allowed_to_write)){
        $plannedOperations += [ordered]@{ operation='WOULD_TOUCH_ALLOWED_FILE'; path=[string]$w; executed=$false }
      }
    }
  }
  return [ordered]@{
    schema='build_task_bounded_executor_v1'
    ...
    proof_written_by_executor=$false
    rollback_needed=$false
    rollback_performed=$false
    forbidden_surface_touched=$false
    allowed_files_snapshot=if($contract){@($contract.files_allowed_to_write)}else{@()}
    forbidden_files_snapshot=if($contract){@($contract.files_forbidden_to_write)}else{@()}
    next_required_step=if($gateAllows){'BUILD_TASK_BOUNDED_EXECUTOR_V2_ALLOWED_DRY_RUN_TO_VALIDATED_WRITE'}else{'AUTHORIZE_OR_REWRITE_CONTRACT_BEFORE_EXECUTION'}
    boundary=[ordered]@{
      executor_only=$true
...
function New-RefocusSeedDiversification($RunId,$RefocusToNewThoughtSeed,$RecentThoughtRepetitionPattern,$NewThoughtSeedToActiveGoal){
  ...
  $branches=@(
    ...
    [ordered]@{ lens='causal_chain'; question='What causal chain makes this loop reappear, and which upstream cause should be inspected first?'; depth_level=3; novelty=0.74 },
    [ordered]@{ lens='validation_criterion'; question='What proof would show that the next thought is stronger rather than just different?'; depth_level=3; novelty=0.70 },
...
function New-ThoughtDepthLadder($RunId,$RefocusSeedDiversification,$DynamicMemoryRetrievalBudget){
  $depth=1
  $steps=@([ordered]@{ level=1; name='detect_repeat'; status='done' })
  if($RefocusSeedDiversification -and $RefocusSeedDiversification.diversification_needed){
    $depth=3
    $steps += [ordered]@{ level=2; name='split_seed_into_branches'; status='done'; branch_count=$RefocusSeedDiversification.branch_count }
    $steps += [ordered]@{ level=3; name='select_non_repeating_lens'; status='done'; selected_lens=$RefocusSeedDiversification.selected_lens }
  }
  if($RefocusSeedDiversification -and $RefocusSeedDiversification.selected_lens -eq 'source_gap'){
    $depth=4
    $steps += [ordered]@{ level=4; name='mark_source_gap_without_external_access'; status='done' }
  }
  return [ordered]@{ schema='thought_depth_ladder_v1'; status='PASS_THOUGHT_DEPTH_LADDER_V1'; run_id=$RunId; depth_level=[int]$depth; steps=@($steps); selected_lens=if($RefocusSeedDiversification){$RefocusSeedDiversification.selected_lens}else{$null}; memory_budget_target=if($DynamicMemoryRetrievalBudget){$DynamicMemoryRetrievalBudget.target_count}else{$null}; boundary=[ordered]@{ thinking_only=$true; depth_measure_only=$true; no_action_execution=$true; no_repo_mutation=$true; no_active_memory_write=$true; no_external_access=$true } }
}
function Get-LatestRefocusToNewThoughtSeed([string]$OutputRoot,[string]$CurrentRunRoot){
  ...
  foreach($d in $dirs){
    if($currentFull -and $d.FullName -eq $currentFull){ continue }
    foreach($seedFile in @('refocus_seed_diversification.json','refocus_to_new_thought_seed.json')){
      $seedPath=Join-Path $d.FullName $seedFile
      $seed=Read-JsonSafe $seedPath
      if($seed -and $seed.new_thought_seed -and -not [string]::IsNullOrWhiteSpace([string]$seed.new_thought_seed.question)){
        $ok=($seed.status -eq 'PASS_REFOCUS_SEED_DIVERSIFICATION_V1' -or $seed.status -eq 'PASS_REFOCUS_TO_NEW_THOUGHT_SEED_V1')
        if($ok){ return [ordered]@{ status='FOUND_REFOCUS_THOUGHT_SEED'; seed=$seed; source_run=$d.Name; source_path=$seedPath; source_kind=$seedFile } }
      }
    }
  }
...
$mentalFrontierRouterPath = Join-Path $runRoot 'mental_frontier_router.json'
$mentalFrontierRouter = Get-MentalFrontierRouter $mentalFrontierExpansionGate
Write-CleanJson $mentalFrontierRouterPath $mentalFrontierRouter 20
...
$dynamicMemoryRetrievalBudgetPath = Join-Path $runRoot 'dynamic_memory_retrieval_budget.json'
...
$proof=[ordered]@{
  ...
  dynamic_memory_retrieval_budget=$dynamicMemoryRetrievalBudget
  ...
  decision_trace=@(
    ...
    [ordered]@{ step='dynamic_memory_retrieval_budget'; result=$dynamicMemoryRetrievalBudget.target_count; proof='Memory retrieval budget is selected from active goal and wake reflex context before compact memory retrieval.' },
...
Write-Host "DEFAULT_WAKE_REFLEXES_STATUS=$($proof.default_wake_reflexes.status)"
Write-Host "DEFAULT_WAKE_REFLEXES_REF=$($proof.default_wake_reflexes_path)"
