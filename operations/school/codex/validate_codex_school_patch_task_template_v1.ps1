...
foreach($field in @('topic_key','depth_level','prerequisite_depth','target_depth','source_basis','source_missing','knowledge_kind','evidence_statement','expected_behavior','validator','proof_requirements','negative_case','return_to_parent','digest_hint')){ if(@($task.required_candidate_fields) -notcontains $field){ $fail += "MISSING_REQUIRED_FIELD:$field" } }
foreach($needle in @('PREFLIGHT_PASS','files_changed_before_preflight','do not mutate active compact memory','single topic only','candidate_limit','validator','negative_case','return_to_parent','source_basis','knowledge_kind','evidence_statement','declarative knowledge','INSUFFICIENT_EVIDENCE_FOR_EXACT_COUNT')){ if($md -notmatch [regex]::Escape($needle)){ $fail += "TASK_MD_MISSING_TEXT:$needle" } }
if(@($task.acceptance_contract).Count -lt 8){ $fail += 'ACCEPTANCE_CONTRACT_TOO_THIN' }
if(-not(@($task.acceptance_contract) -match 'non-empty source_basis')){ $fail += 'GROUNDED_SOURCE_CONTRACT_MISSING' }
if(-not(@($task.acceptance_contract) -match 'declarative knowledge')){ $fail += 'DECLARATIVE_KNOWLEDGE_CONTRACT_MISSING' }
if(-not(@($task.acceptance_contract) -match 'insufficient distinct evidence')){ $fail += 'INSUFFICIENT_EVIDENCE_FAIL_CONTRACT_MISSING' }
