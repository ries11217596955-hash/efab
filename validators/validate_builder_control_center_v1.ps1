...
$list=& $ctl -Mode List -Json|ConvertFrom-Json;if(@($list).Count -ne 19){throw 'LIST_ACTION_COUNT_BAD'}
...
$diag=@('runtime.diagnose','memory.diagnose','inventory.diagnose','capability.diagnose','control_center.diagnose','builder.preflight','builder.candidate.status','builder.acceptance.plan');$dp=& $ctl -Mode Plan -Action $diag -Json|ConvertFrom-Json
if($dp.status-ne'READY'-or[int]$dp.diagnostic_count-ne8-or[int]$dp.live_mutation_count-ne0-or$dp.execution_mode-ne'SEQUENTIAL'){throw 'DIAG_PLAN_BAD'}
$dr=& $ctl -Mode Run -Action $diag -Json|ConvertFrom-Json;if($dr.status-ne'PASS_DIAGNOSTIC_RUN'-or@($dr.results).Count-ne8){throw 'DIAG_RUN_BAD'}
...
$cc=@($dr.results|Where-Object id -eq 'control_center.diagnose')|Select-Object -First 1;if($cc.status-ne'DIAGNOSTIC_PASS'-or[int]$cc.action_count-ne19){throw 'CONTROL_DIAG_BAD'}
...
Write-Output ('PASS_BUILDER_CONTROL_CENTER_V4|ACTIONS=19|DIAGNOSE_ROUTES=8|OVERVIEW=PASS|REMOTE_SCOPE=PASS|MULTI_DIAG=PASS|LIVE_MUTATION=0|TRACKED_MUTATION=0|RUNTIME_STARTED=0')
