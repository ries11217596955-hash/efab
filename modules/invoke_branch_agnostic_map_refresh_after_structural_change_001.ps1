...
$missingRequiredComponents = @($components | Where-Object { $_.status -ne 'PRESENT_ON_CURRENT_BRANCH' -or -not $_.required_files_present } | ForEach-Object { $_.id })
...
$activeMap = [ordered]@{
  ...
  boundary = 'This is the single canonical composition/status map. It does not describe how to invoke capabilities; capability invocation belongs to a separate capability map.'
}
$bodyMap = [ordered]@{
  ...
  components = @($components | ForEach-Object { [ordered]@{ id=$_.id; status=$_.status; root=$_.root; file_count=$_.file_count; script_count=$_.script_count; latest_runtime_artifact=$_.latest_runtime_artifact; required_files_present=$_.required_files_present } })
}
...
$result.build_result = [ordered]@{ result = $(if ($missingRequiredComponents.Count -eq 0) { 'PASS' } else { 'PASS_WITH_MISSING_COMPONENTS' }); builder = 'AGENT_BODY_COMPOSITION_MAP_V1'; canonical_composition_map = $result.canonical_composition_map; component_count = $components.Count; structural_path_count = $structuralFiles.Count }
