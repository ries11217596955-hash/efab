...
$PackRoot = Join-Path $RepoRoot "packs\PHASE71_AGENT_PROGRAM_EXECUTOR_V1"
$ValidatorPayloadPath = Join-Path $PackRoot "payload\validators\validate_agent_program_executor_v1.ps1"
$ValidatorTargetPath = Join-Path $RepoRoot "validators\validate_agent_program_executor_v1.ps1"
$SourceProgramPath = "agent_programs/remediation_intake_operator_agent_v1/PROGRAM.json"
$RunRootPath = "agent_program_runs/remediation_intake_operator_agent_v1"
...
$ExecutionPlan = [ordered]@{
    plan_id = "AGENT_PROGRAM_EXECUTION_PLAN_REMEDIATION_INTAKE_OPERATOR_AGENT_V1"
    status = "READY"
    ...
    conclusion = "Builder can read the agent production program and prepare an execution plan."
}
...
Проверки взяты из `validation_requirements` программы:
...
Invoke-NativeGitCommand -Label "ADD" -Arguments @(
    ...
    ".\TASK_QUEUE.json",
    ".\packs\registry.json",
    ".\tasks\TASK_AGENT_PROGRAM_EXECUTOR_V1_001.json",
    ".\packs\PHASE71_AGENT_PROGRAM_EXECUTOR_V1",
    ".\agent_program_runs\remediation_intake_operator_agent_v1",
...
Invoke-NativeGitCommand -Label "COMMIT" -Arguments @(
    "commit",
    "-m",
    "Add agent production program executor v1"
)
