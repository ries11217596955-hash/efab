...
$cc=@($dr.results|Where-Object id -eq 'control_center.diagnose')|Select-Object -First 1;if($cc.status-ne'DIAGNOSTIC_PASS'-or[int]$cc.action_count-ne25){throw 'CONTROL_DIAG_BAD'}
...
try{
 ...
 $dummySchool=$null
 try{$dummySchool=Start-Process -FilePath 'cmd.exe' -ArgumentList @('/c','ping 127.0.0.1 -n 8 >nul & rem run_agent_school.ps1') -PassThru -WindowStyle Hidden;Start-Sleep -Milliseconds 500;$dupPlan=& $resumeCtl -Mode Plan -Action @('school.resume') -Json|ConvertFrom-Json;$dupReady=@($dupPlan.readiness|Where-Object id -eq 'school.resume')|Select-Object -First 1;if($dupReady.status-ne'ALREADY_RUNNING'){throw ('SCHOOL_RESUME_DUPLICATE_RUNTIME_GATE_BAD:'+$dupReady.status)}}finally{if($null-ne$dummySchool -and -not$dummySchool.HasExited){Stop-Process -Id $dummySchool.Id -Force -ErrorAction SilentlyContinue;try{$dummySchool.WaitForExit(3000)}catch{}}}
...
}finally{if($resumePushed){Pop-Location};if(Test-Path $resumeLab){Remove-Item $resumeLab -Recurse -Force}}
