...
elseif($Mode -eq 'Plan'){
 ...
 $readiness=@();foreach($a in $selected){if([bool]$a.read_only){$readiness+=,[ordered]@{id=$a.id;status='READY_READ_ONLY'}}else{$readiness+=,(Get-StartReadiness $a.id)}}
 $blocked=@($readiness|Where-Object{$_.status -notin @('READY','READY_READ_ONLY')})
 $out=[ordered]@{status=if($conflicts.Count){'BLOCKED_CONFLICT'}elseif($blocked.Count){'BLOCKED'}else{'READY'};selected=@($selected.id);count=$selected.Count;mutation_count=@($selected|Where-Object{-not[bool]$_.read_only}).Count;parallel_safe=(@($selected|Where-Object{-not[bool]$_.parallel_safe}).Count -eq 0);conflicts=@($conflicts);readiness=@($readiness)}
}
else{
 if(-not$Action.Count){throw'CONTROL_CENTER_ACTION_REQUIRED'}
 ...
 if($plan.status -ne 'READY'){ $out=[ordered]@{status='NOT_STARTED';plan=$plan} }
   ...
   $results=@();foreach($id in $Action){$a=Get-Reg $id;if([bool]$a.read_only){$results+=,(Invoke-Observed $id)}else{$results+=,(Invoke-Start $id)}}
   $out=[ordered]@{status=if(@($results|Where-Object{$_.status -eq 'STARTED'}).Count){'START_DISPATCHED'}elseif(@($results|Where-Object{$_.status -eq 'BLOCKED_CONFIRMATION_REQUIRED'}).Count){'BLOCKED_CONFIRMATION_REQUIRED'}else{'PASS_READ_ONLY_RUN'};selected=@($Action);results=@($results)}
 }
}
