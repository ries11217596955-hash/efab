...
Assert ([int]$model.counts.triage_items -eq (CountOf $items)) 'MODEL_TRIAGE_COUNT_BAD'
...
Assert ($proof.persistent_not_temporary -eq $true) 'PROOF_NOT_TEMPORARY_FALSE'
...
Assert ($model.persistent_contract.no_active_promotion_without_passport_validator_proof -eq $true) 'MODEL_ACTIVE_GATE_BAD'
Assert ($model.persistent_contract.no_full_passport_generation_for_all_candidates -eq $true) 'MODEL_FULL_PASSPORT_BOUNDARY_BAD'
Assert ($model.persistent_contract.no_candidate_accepted_as_organ_from_lanes_alone -eq $true) 'MODEL_ACCEPTANCE_BOUNDARY_BAD'
Assert ($model.persistent_contract.live_process_touched -eq $false) 'MODEL_LIVE_TOUCH_BAD'
$badActive=@($decisions|Where-Object{IsTrueValue $_.active_allowed})
Assert ((CountOf $badActive) -eq 0) 'DECISION_ACTIVE_ALLOWED_PRESENT'
$missingLane=@($decisions|Where-Object{[string]::IsNullOrWhiteSpace([string]$_.lane)})
Assert ((CountOf $missingLane) -eq 0) 'DECISION_MISSING_LANE'
$missingGates=@($decisions|Where-Object{(CountOf $_.required_gates) -eq 0})
Assert ((CountOf $missingGates) -eq 0) 'DECISION_MISSING_GATES'
$fast=@($decisions|Where-Object{$_.lane -eq 'FAST_LANE_PASSPORT_DRAFT'})
Assert ((CountOf $fast) -ge 1) 'NO_FAST_LANE_CALIBRATION_CANDIDATE'
Assert (@($fast|Where-Object{$_.candidate_id -eq 'contracts_accepted_atom_retention_organ'}).Count -eq 1) 'ACCEPTED_ATOM_NOT_FAST_LANE'
$unique=(CountOf ($decisions.candidate_id|Sort-Object -Unique))
Assert ($unique -eq (CountOf $decisions)) 'DUPLICATE_DECISION_IDS'
Assert ((CountOf @($model.lanes)) -ge 4) 'LANE_GROUPS_TOO_FEW'
