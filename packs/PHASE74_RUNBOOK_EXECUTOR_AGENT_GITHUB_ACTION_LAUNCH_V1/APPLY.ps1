...
$PackRoot = Join-Path $RepoRoot "packs/PHASE74_RUNBOOK_EXECUTOR_AGENT_GITHUB_ACTION_LAUNCH_V1"
$WorkflowPayloadPath = Join-Path $PackRoot "payload/.github/workflows/run-runbook-executor-agent-v1.yml"
$ValidatorPayloadPath = Join-Path $PackRoot "payload/validators/validate_runbook_executor_agent_github_action_launch_v1.ps1"
$WorkflowTargetDirectory = Join-Path $RepoRoot ".github/workflows"
$WorkflowTargetPath = Join-Path $WorkflowTargetDirectory "run-runbook-executor-agent-v1.yml"
$ValidatorTargetPath = Join-Path $RepoRoot "validators/validate_runbook_executor_agent_github_action_launch_v1.ps1"
...
Invoke-NativeGitCommand -Label "ADD" -Arguments @(
  ...
  ".\TASK_QUEUE.json",
  ".\packs\registry.json",
  ".\tasks\TASK_RUNBOOK_EXECUTOR_AGENT_GITHUB_ACTION_LAUNCH_V1_001.json",
  ".\packs\PHASE74_RUNBOOK_EXECUTOR_AGENT_GITHUB_ACTION_LAUNCH_V1",
  ...
  ".\proofs\RUNBOOK_EXECUTOR_AGENT_GITHUB_ACTION_LAUNCH_V1.json",
  ".\reports\external_agent_production\RUNBOOK_EXECUTOR_AGENT_GITHUB_ACTION_LAUNCH_V1_REPORT.json"
)

Invoke-NativeGitCommand -Label "COMMIT" -Arguments @(
  "commit",
  "-m",
  "Add GitHub Action launch for runbook executor agent"
)
