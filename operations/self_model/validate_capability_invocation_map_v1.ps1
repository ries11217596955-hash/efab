...
$validatorAliasMap=@{
 ...
 'self_build_program_generator_v1'='packs/PHASE88_SELF_BUILD_PROGRAM_GENERATOR_V1/VALIDATE.ps1'
...
$noIdGap=@($m.gaps|Where-Object gap_id -eq 'TASKS_WITHOUT_CAPABILITY_ID')|Select-Object -First 1;Assert ($null-ne$noIdGap) 'NO_ID_GAP_MISSING';$gapTaskRefs=@();if($null-ne$noIdGap.items){$gapTaskRefs=@($noIdGap.items|ForEach-Object{[string]$_.source_task_ref}|Where-Object{-not[string]::IsNullOrWhiteSpace($_)}|Sort-Object)};Assert ($gapTaskRefs.Count -eq $withoutId.Count) 'NO_ID_TASK_COUNT_MISMATCH';if($withoutId.Count -gt 0){Assert (@(Compare-Object ($withoutId|Sort-Object) $gapTaskRefs).Count -eq 0) 'NO_ID_TASK_SET_MISMATCH'}
