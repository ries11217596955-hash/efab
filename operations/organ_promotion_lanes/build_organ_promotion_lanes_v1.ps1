...
function LaneFor($item){
  ...
  if($pr -eq 'PASSPORT_DRAFT_VALIDATED_BLOCKED_RUNTIME_PROOF'){ return 'CALIBRATED_PASSPORT_DRAFT_BLOCKED_RUNTIME' }
...
function RequiredGate($lane){
  switch($lane){
    ...
    'CALIBRATED_PASSPORT_DRAFT_BLOCKED_RUNTIME' { return @('passport_draft_validated','runtime_or_micro_proof_blocker_recorded','no_active_without_missing_proof_restored','owner_route_acceptance_required') }
...
foreach($item in ($items|Sort-Object candidate_id)){
  ...
  [bool]$promotion_allowed=$false; [bool]$passport_allowed=$false; [bool]$active_allowed=$false
  if($lane -eq 'FAST_LANE_PASSPORT_DRAFT'){$decision='DRAFT_PASSPORT_CANDIDATE';$passport_allowed=$true}
  elseif($lane -eq 'CALIBRATED_PASSPORT_DRAFT_BLOCKED_RUNTIME'){$decision='KEEP_DRAFT_BLOCKED_UNTIL_RUNTIME_PROOF'}
  ...
  $decisions += [pscustomobject][ordered]@{candidate_id=[string]$item.candidate_id;path=[string]$item.path;triage_class=[string]$item.triage_class;passport_readiness=[string]$item.passport_readiness;lane=[string]$lane;decision=[string]$decision;promotion_allowed=$promotion_allowed;passport_draft_allowed=$passport_allowed;active_allowed=$active_allowed;required_gates=@(RequiredGate $lane);reason=[string]$item.reason;validator_refs=@($item.validator_refs);proof_refs=@($item.proof_refs)}
}
...
$batchPolicy=@(
 [pscustomobject][ordered]@{lane='FAST_LANE_PASSPORT_DRAFT';batch_action='generate_or_normalize_passport_draft_one_at_a_time_until_validator_template_is_proven';acceptance='passport draft + dedicated validator + proof; no active passport';current_strategy='use next ready candidate after accepted_atom calibration'}
 [pscustomobject][ordered]@{lane='CALIBRATED_PASSPORT_DRAFT_BLOCKED_RUNTIME';batch_action='keep draft passport; block activation until missing runtime/micro proof is restored';acceptance='passport validator pass + calibration proof + blocker record; no active passport'}
...
$model=[pscustomobject][ordered]@{
 schema='ORGAN_PROMOTION_LANES_V1';status='PASS_ORGAN_PROMOTION_LANES_V1';role='persistent_growth_gate';purpose='Convert body-map candidate triage into repeatable promotion lanes so Builder does not repair candidates one by one or mistake material for organs.';source_map=$mapPath;source_triage=$triagePath;source_promotion_plan=$planPath;source_passport_index=$passportIndexPath
 counts=[pscustomobject][ordered]@{confirmed_components=CountOf $map.confirmed_components;source_candidates=CountOf $map.primary_evidence_candidates;triage_items=CountOf $items;lane_decisions=CountOf $decisions;lanes=CountOf $laneGroups;fast_lane_passport_draft=CountOf $fast;calibrated_passport_draft_blocked_runtime=CountOf $calibrated;owner_link_required=CountOf $ownerLink;review_lane=CountOf $review;material_or_archive=CountOf $materialOrArchive;passport_index_count=$(if($passportIndex.passport_count){[int]$passportIndex.passport_count}else{CountOf $passportIndex.passports});active_passports=$(if($passportIndex.active_count -ne $null){[int]$passportIndex.active_count}else{0})}
 lanes=@($laneGroups);batch_policy=@($batchPolicy);lane_decisions=@($decisions);calibration_candidate=$(if(CountOf $calibrated -gt 0){$calibrated[0].candidate_id}else{$null})
 persistent_contract=[pscustomobject][ordered]@{not_temporary=$true;first_run_handles_current_candidates=$true;future_role='classify_new_body_surfaces_into_promotion_lanes_before_any passport or organ claim';no_active_promotion_without_passport_validator_proof=$true;no_full_passport_generation_for_all_candidates=$true;no_candidate_accepted_as_organ_from_lanes_alone=$true;no_live_claim_created=$true;live_process_touched=$false}
 next_recommended_actions=@('Restore/generate missing accepted_atom micro-proof before any runtime/active claim.','Use remaining FAST_LANE candidates only after calibration boundary is accepted.','Batch-block OWNER_LINK_REQUIRED items until parent organ/Owner decision exists.')
...
$proof=[pscustomobject][ordered]@{schema='organ_promotion_lanes_v1_proof';status='PASS_ORGAN_PROMOTION_LANES_V1';model_path=$modelPath;report_path=$reportPath;source_candidate_count=CountOf $map.primary_evidence_candidates;triage_count=CountOf $items;decision_count=CountOf $decisions;all_candidates_have_lane=((CountOf $decisions) -eq (CountOf $items) -and (CountOf $items) -eq (CountOf $map.primary_evidence_candidates));unique_candidate_ids=((CountOf ($decisions.candidate_id|Sort-Object -Unique)) -eq (CountOf $decisions));fast_lane_count=CountOf $fast;calibrated_passport_draft_blocked_runtime_count=CountOf $calibrated;owner_link_required_count=CountOf $ownerLink;review_lane_count=CountOf $review;material_or_archive_count=CountOf $materialOrArchive;no_candidate_active_allowed=((CountOf $activeAllowed) -eq 0);no_lane_accepts_organ_without_gates=$true;persistent_not_temporary=$model.persistent_contract.not_temporary;cortex_refs_in_lanes=CountOf $cortexRefs;live_process_touched=$false;created_at=(Get-Date).ToString('o')}
...
$md=@('# Organ Promotion Lanes V1','','status: PASS_ORGAN_PROMOTION_LANES_V1','','Purpose: persistent growth gate. It turns body-map candidate triage into lanes so Builder does not process all candidates manually or promote materials as organs.','','Counts:',('- source candidates: '+(CountOf $map.primary_evidence_candidates)),('- lane decisions: '+(CountOf $decisions)),('- lanes: '+(CountOf $laneGroups)),('- fast lane passport draft: '+(CountOf $fast)),('- calibrated passport draft blocked runtime: '+(CountOf $calibrated)),('- owner link required: '+(CountOf $ownerLink)),('- review lane: '+(CountOf $review)),('- material/archive: '+(CountOf $materialOrArchive)),'','Boundary:','- lanes are not organ acceptance.','- no active passport is created.','- no live claim is created.','- no full passport generation for all candidates.','- accepted_atom_retention_organ is calibrated but blocked by missing runtime/micro proof.','','Next: restore/regenerate accepted atom micro-proof or continue with remaining fast-lane candidate under same boundary.')
