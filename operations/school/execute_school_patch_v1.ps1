...
if($ExecutorMode -eq 'DryRun'){
  ...
  for($i=1;$i -le [int]$task.candidate_limit;$i++){
    ...
    $obj=[ordered]@{
      ...
      source_basis=@('operations/school/execute_school_patch_v1.ps1')
      source_missing=$false
      claim="The mock topic $($task.topic_key) has validator depth $depth for candidate $i."
      knowledge_kind='observation'
      evidence_statement='Mock validator source establishes this synthetic observation for validator-only use.'
