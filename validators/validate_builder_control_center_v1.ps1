...
$list=& $ctl -Mode List -Json|ConvertFrom-Json;if(@($list).Count -ne 20){throw 'LIST_ACTION_COUNT_BAD'}
...
$cc=@($dr.results|Where-Object id -eq 'control_center.diagnose')|Select-Object -First 1;if($cc.status-ne'DIAGNOSTIC_PASS'-or[int]$cc.action_count-ne20){throw 'CONTROL_DIAG_BAD'}
...
Write-Output ('PASS_BUILDER_CONTROL_CENTER_V4|ACTIONS=20|DIAGNOSE_ROUTES=9|OVERVIEW=PASS|REMOTE_SCOPE=PASS|MULTI_DIAG=PASS|LIVE_MUTATION=0|TRACKED_MUTATION=0|RUNTIME_STARTED=0')
