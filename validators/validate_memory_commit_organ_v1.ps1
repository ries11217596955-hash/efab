...
foreach($needle in @('DrainAgentLife','PostSchoolComplete','DeleteRejected','Validate-Packet','merge_compact_memory_intake_queue_v1.ps1','AuditSchoolQuality','PruneProcessedAgentLife','LOW_VARIETY_VALID_SCAFFOLD')){ if($scriptText -notlike "*$needle*"){ Add-Err "controller_missing:$needle" } }
if($policy){
...
if($RequireDrain){
  if(-not $proof){ Add-Err 'drain_proof_required' }
    ...
    if($proof.status -ne 'PASS_MEMORY_COMMIT_DRAIN_AGENTLIFE_V1'){ Add-Err "drain_status_bad:$($proof.status)" }
    if($proof.queue_after -ne 0){ Add-Err "queue_after_not_zero:$($proof.queue_after)" }
    if($proof.accepted_count -lt 1){ Add-Err 'accepted_count_lt_1' }
