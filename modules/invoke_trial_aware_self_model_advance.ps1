function Read-JsonOptional {
  param([string]$Path)
  if (-not (Test-Path $Path)) { return $null }
  try { return Get-Content $Path -Raw | ConvertFrom-Json } catch { return $null }
}

function Write-JsonFile {
  param($Path, $Object, [int]$Depth = 20)
  $dir = Split-Path $Path -Parent
  if ($dir -and -not (Test-Path $dir)) { New-Item -ItemType Directory -Force -Path $dir | Out-Null }
  $Object | ConvertTo-Json -Depth $Depth | Set-Content -Path $Path -Encoding UTF8
}

function Invoke-TrialAwareSelfModelAdvance {
  param(
    [string]$RepoRoot,
    [string]$RunId,
    $Entry,
    [string]$OutputRoot
  )

  Push-Location $RepoRoot

  try {
    if (-not (Test-Path $OutputRoot)) {
      New-Item -ItemType Directory -Force -Path $OutputRoot | Out-Null
    }

    $SelfModelPath = "self_model/BUILDER_SELF_MODEL.json"
    $SelfModel = Read-JsonOptional $SelfModelPath
    $Controller = Read-JsonOptional "self_control/AUTONOMOUS_LOOP_CONTROLLER.json"
    $P124 = Read-JsonOptional "proofs/self_development/PHASE124_BUILD_SELF_MODEL_FIRST_RUNTIME_ENTRYPOINT_V1.json"
    $P125 = Read-JsonOptional "proofs/self_development/PHASE125_RUN_SELF_MODEL_FIRST_CONTROLLER_GOVERNED_TRIAL_V1.json"

    $status = "BLOCKED"
    $closedNeed = ""
    $currentNeed = "NEED_TRIAL_AWARE_SELF_MODEL_REVIEW"
    $nextStep = "PHASE127_REVIEW_TRIAL_AWARE_SELF_MODEL_INPUTS_V1"
    $reason = ""

    if (
      $Entry.status -eq "PASS" -and
      $Entry.current_need -eq "NEED_CONTROLLER_GOVERNED_SELF_BUILD_TRIAL" -and
      $SelfModel.status -eq "PASS" -and
      $Controller.status -eq "PASS" -and
      $P124.status -eq "PASS" -and
      $P125.status -eq "PASS" -and
      $P125.classification -eq "SELF_MODEL_FIRST_CONTROLLER_GOVERNED_TRIAL_PASS"
    ) {
      $status = "PASS"
      $closedNeed = "NEED_CONTROLLER_GOVERNED_SELF_BUILD_TRIAL"
      $currentNeed = "NEED_SELF_BUILD_OPERATION_CONTRACT"
      $nextStep = "PHASE127_BUILD_SELF_BUILD_OPERATION_CONTRACT_V1"
      $reason = "PHASE125 proved the self-model-first controller-governed trial. Self-model can close the trial need and move to a formal self-build operation contract."
    } else {
      $status = "BLOCKED"
      $reason = "Entry, controller, self-model, PHASE124 proof, or PHASE125 proof is missing/unexpected."
    }

    $oldClosed = @()
    if ($null -ne $SelfModel -and $null -ne $SelfModel.closed_needs) { $oldClosed = @($SelfModel.closed_needs) }
    $closedNeeds = @($oldClosed + $closedNeed | Where-Object { $_ -and $_.Trim() -ne "" } | Sort-Object -Unique)

    $oldCaps = @()
    if ($null -ne $SelfModel -and $null -ne $SelfModel.proven_capabilities) { $oldCaps = @($SelfModel.proven_capabilities) }

    $Capabilities = @($oldCaps)
    $Capabilities += [ordered]@{ id = "SELF_MODEL_FIRST_RUNTIME_ENTRYPOINT"; status = if ($P124.status -eq "PASS") { "PROVEN" } else { "UNKNOWN" }; proof = "PHASE124_BUILD_SELF_MODEL_FIRST_RUNTIME_ENTRYPOINT_V1" }
    $Capabilities += [ordered]@{ id = "SELF_MODEL_FIRST_CONTROLLER_GOVERNED_TRIAL"; status = if ($P125.status -eq "PASS") { "PROVEN" } else { "UNKNOWN" }; proof = "PHASE125_RUN_SELF_MODEL_FIRST_CONTROLLER_GOVERNED_TRIAL_V1" }

    $UpdatedSelfModel = [ordered]@{
      status = $status
      self_model_id = "BUILDER_SELF_MODEL_V3_TRIAL_AWARE"
      updated_by = "TRIAL_AWARE_SELF_MODEL_ADVANCE_V1"
      run_id = $RunId
      active_line = "AGENT_BUILDER / SELF_BUILD"
      identity = "primitive_brain_cell_with_hands"
      local_first = $true
      external_brain_dependency = $false
      current_state_summary = "Builder has a self-model-first controller-governed trial path and no longer needs to replay legacy organ-build routes."
      proven_capabilities = $Capabilities
      closed_needs = $closedNeeds
      current_detected_need = $currentNeed
      current_missing_capability = "SELF_BUILD_OPERATION_CONTRACT"
      recommended_next_step = $nextStep
      trial_aware = $true
      source_phase125_classification = if ($null -ne $P125) { $P125.classification } else { "__MISSING__" }
      autonomy_claimed = $false
      codex_used = $false
      main_touched = $false
    }

    Write-JsonFile $SelfModelPath $UpdatedSelfModel 30

    $Output = [ordered]@{
      status = $status
      engine_name = "TRIAL_AWARE_SELF_MODEL_ADVANCE_V1"
      run_id = $RunId
      self_model_path = $SelfModelPath
      self_model_updated = ($status -eq "PASS")
      closed_need = $closedNeed
      current_detected_need = $currentNeed
      proposed_next_step = $nextStep
      reason = $reason
      queue_mutated = $false
      autonomy_claimed = $false
      codex_used = $false
      main_touched = $false
    }

    $OutputPath = Join-Path $OutputRoot "TRIAL_AWARE_SELF_MODEL_ADVANCE_OUTPUT.json"
    Write-JsonFile $OutputPath $Output 20
    $Output["output_path"] = $OutputPath

    return [pscustomobject]$Output
  } finally {
    Pop-Location
  }
}
