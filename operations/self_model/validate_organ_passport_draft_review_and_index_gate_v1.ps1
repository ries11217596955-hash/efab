...
Assert (@($idx.passports).Count -eq $items.Count) 'INDEX_COUNT_MISMATCH'
foreach($it in $items){
  ...
  Assert ($pass.maturity -eq 'DRAFT') ("MATURITY_MUTATED:{0}" -f $it.organ_id)
  Assert ($pass.live_or_lab_status -eq 'NOT_PROVEN') ("LIVE_STATUS_MUTATED:{0}" -f $it.organ_id)
  Assert ($it.active_claim_detected -eq $false) ("ACTIVE_CLAIM_DETECTED:{0}" -f $it.organ_id)
  Assert ($it.live_claim_detected -eq $false) ("LIVE_CLAIM_DETECTED:{0}" -f $it.organ_id)
  Assert (@($it.required_fields_missing).Count -eq 0) ("REQUIRED_FIELDS_MISSING:{0}" -f $it.organ_id)
  Assert (@($it.owned_files_missing).Count -eq 0) ("OWNED_FILES_MISSING:{0}" -f $it.organ_id)
}
Assert (@($items|Where-Object{$_.organ_id -eq 'operations_self_model' -and $_.lab_validation_candidate -eq $true}).Count -eq 1) 'OPERATIONS_SELF_MODEL_NOT_LAB_CANDIDATE'
...
Assert ($r.boundaries.no_live_claims_created -eq $true) 'LIVE_BOUNDARY_BAD'
Assert ($p.no_active_passports_created -eq $true) 'PROOF_ACTIVE_FALSE'
Assert ($p.no_live_claims -eq $true) 'PROOF_LIVE_FALSE'
