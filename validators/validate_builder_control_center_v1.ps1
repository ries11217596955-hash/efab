...
$ctlText=Get-Content $ctl -Raw;if(-not$ctlText.Contains('delivery_attempted=$false')-or$ctlText.Contains('api.telegram.org')){throw 'SCHOOL_NOTIFICATION_PLAN_NO_SEND_BOUNDARY_BAD'}
...
$list=& $ctl -Mode List -Json|ConvertFrom-Json;if(@($list).Count -ne 23){throw 'LIST_ACTION_COUNT_BAD'}
...
$cc=@($dr.results|Where-Object id -eq 'control_center.diagnose')|Select-Object -First 1;if($cc.status-ne'DIAGNOSTIC_PASS'-or[int]$cc.action_count-ne23){throw 'CONTROL_DIAG_BAD'}
...
Write-Output ('PASS_BUILDER_CONTROL_CENTER_V4|ACTIONS=23|DIAGNOSE_ROUTES=11|OVERVIEW=PASS|REMOTE_SCOPE=PASS|MULTI_DIAG=PASS|LIVE_MUTATION=0|TRACKED_MUTATION=0|RUNTIME_STARTED=0')
